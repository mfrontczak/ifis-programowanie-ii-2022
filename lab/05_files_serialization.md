# Lab 5
## Obsługa plików
Do obsługi plików otwarcia(r)/zapisu(w)/nadpisania(a) służy funkcja `open`. Funkcja `open` działa również z wyrażeniem `with`. Słowo kluczowe `with` służy do zarządzania zasobami (np. plikami), pozwala nam zachować dużo czytelniejszą strukturę kodu oraz daje gwarancję, że dany zasób zostanie zwolniony nawet w przypadku wystąpienia wyjątku. 
...
✏️ Napisz skrypt w którym użytkownik poda dowolną ilość liczb a następnie zapisze je do pliku w trybie binarnym. Spróbuj go odczytać w Notatniku.
...
⚠️ Pliki do przykładów znajdują się w katalogu [/materiały/lab05](/materiały/lab05).
